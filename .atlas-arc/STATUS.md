...
- **Perf probe (a55f8fd):** engine A recommended (DEC-010, proposed). The desktop budgets pass: keystroke p95 5.0 ms, usable 88 ms after DOMContentLoaded, core shard 102 KB. The phone keystroke p95 fails: 18.0 ms against a 16 ms budget. The next step is extreme-software-optimization driven by the probe's hotspot table: selectTopK allocation, result-list layout, and frame work. Re-measure on a quiet host first.
