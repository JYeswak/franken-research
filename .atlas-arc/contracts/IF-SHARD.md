...
UNK-008 is resolved: on 2026-09-25 the maintainer chose `open` (DEC-011; DEC-008, which proposed a `robots.txt` group disallowing `/assets/search-quotes.js` for GPTBot, ClaudeBot, anthropic-ai, Google-Extended and CCBot, is rejected). The quotes shard ships its quotes, and `site/robots.txt` carries no crawler-specific rule. `search/crawler-policy` records the state (`open`); gate Q3 (HON-15) fails the build if the file is absent or holds anything but `pending`, `open` or `block-ai-lab-quotes`, and checks the shard and `robots.txt` against whichever state is committed. DEC-P06's empty-shard rule still applies if the state is ever set back to `pending`.
