...
7. **Crawlers.** The committed crawler policy is `open` (DEC-011, 2026-09-25): the quotes shard carries quotes and `robots.txt` carries no crawler-specific rule. HON-15 fails the build when `search/crawler-policy` is missing, and would empty the shard again if the maintainer ever set it back to `pending` (DEC-P06).
