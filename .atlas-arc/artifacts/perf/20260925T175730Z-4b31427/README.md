...
> **Correction (2026-09-25, after review 7d): this run does NOT meet REQ-O2.** The budget is keystroke to results *painted*. The 8.0 ms below is `key2layout`, which stops at forced layout, before the frame. The paint metric (`key2paint`) for AL3 on the phone is p95 16.4 / 16.6 ms (FAIL, budget 16) and p99 60.3 / 59.3 ms (FAIL, limit 2 x p95 = 32.8). There were also 2 to 3 Long Tasks over 50 ms per 10 runs (FAIL, the budget is zero). What does stand is the same-invocation win over the incumbent and the golden-diff identity. Round 2 is in `../20260925T192212Z-d29eb1c/`.
>
> A second defect, found in round 2: the rungs AL1 to AL5 did not load the quotes shard (503 KB of JSON parsed before typing), while A did. So heap and GC state while typing did not match between A and the rungs. Round 2 loads the shard for every rung.

**Result as first written, on the layout metric: engine A with levers L1, L2 and L3a (rung `AL3`) cuts phone `key2layout` p95 to 8.0 ms, in both invocations that measured it. On the same invocations the incumbent A measured 16.4 ms and 15.8 ms. Desktop `key2layout` p95 is 1.8 ms. The results are identical to the incumbent's: 0 changed rows over 1,165 probe queries, the 88 relevance golden-set queries (ebac079) and their 4,206 keystroke prefixes, on both shards, with scores compared bit for bit.**
