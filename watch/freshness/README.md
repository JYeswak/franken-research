...
`ops/write-job.mjs` fails W3 when a job lacks the main-only guard; when a checkout persists credentials; when any action is not pinned to a full 40-hex commit SHA; when the workflow sets `env` or `defaults`; when the workflow, or any job but `publish`, holds a permission value other than `read` or `none` (or `read-all`), whatever the scope is called, or `build` names no permissions of its own; when `build` references a secret other than `GITHUB_TOKEN`, runs no gate chain, uploads before it, or uploads a list that differs from `PATHS`; when `publish` does not need `build`, sets job `env`, `defaults`, `container`, `services` or a step `shell`, uses an action other than checkout, setup-node and download-artifact, downloads inside the checkout, runs an install, build or gate command or an interpreter other than node, calls node other than as `node <script> [args]` for `ops/take-build-output.mjs`, `ops/briefs-guard.mjs` or `watch/freshness/dashboard.mjs` (a flag such as `--require` before the script, or node by path, fails), or names a loader variable (`NODE_OPTIONS`, `NODE_PATH`, `LD_*`, `DYLD_*`, `BUN_*`), `$GITHUB_ENV`, `$GITHUB_PATH`, `export`, `env` or a `${{ }}` expression in run text; when a `publish` step sets any env variable but `GITHUB_TOKEN`, or sets or receives the token without being the push or the sync; when a run line writes a credential into the Git config; when `publish` does not apply, guard, push and sync in that order, or pushes after the sync; when a `publish` run body differs, byte for byte after trailing newlines, from the reviewed text for its step name in `PUBLISH_RUNS` (the report names the step and the first differing line), or a `publish` step runs text under a name `PUBLISH_RUNS` does not hold; and when any of those three scripts, or anything they import, imports a package or uses a non-literal `import()` or `require()`. Any change to a `publish` run body therefore needs the same change in `PUBLISH_RUNS`, in the same commit. `ops/take-build-output.mjs` checks every destination before it writes any file, and refuses a path in the checkout that holds a symlink or that exists and is not a regular file. These rules are a closed list (see the Limits note under gate W3 in `site/BUILD-GATES.md`): the job boundary is the control, and a new kind of workflow change needs a new rule.
