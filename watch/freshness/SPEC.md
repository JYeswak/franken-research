...
**FR-C.3** (MUST) A run still in progress, or an API gap, never reads as C3. For `now`, the CI point is the newest default-branch commit whose push-triggered test runs have all completed. It is found in one page of the default branch's runs from the Actions API. HEAD is that point when it qualifies. CI is `unknown` only when no listed commit qualifies. `live.json` records the commit the CI class describes, as `dims.ci.now_commit`, and the card shows it.
