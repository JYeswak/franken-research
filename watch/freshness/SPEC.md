...
**FR-O.6** (MUST) The job that writes (the watch: it pushes commits and syncs the dashboard) runs only on `refs/heads/main`, whether it was triggered by the schedule or dispatched by hand. Its checkout does not persist credentials. The token reaches only the steps that need it: the watch's GitHub API reads, the push, and the dashboard sync. No install, build or gate step runs with a write-capable token in its environment or in the Git config. The check covers every way a step can receive the token: `secrets.GITHUB_TOKEN`, `secrets['GITHUB_TOKEN']` and `github.token`, in a `run` step or a `uses` step, and in step, job or workflow env or `with:`. No `git push` may follow the dashboard sync.
