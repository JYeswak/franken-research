...
- **Order of the scheduled run** (FR-D.5): watch (`--apply`), build (cards, feed, harness report), gate chain, commit, push, and only then the dashboard sync, from the committed `watch/live.json`. A failed gate or push leaves the issue as it was; a failed sync fails the run but leaves the pushed files in place.
- **Main only, no stored token** (FR-O.6): the scheduled job runs only on `main` (a manual dispatch from another branch is skipped), its checkout keeps no token in `.git/config`, and the token is in the env of three steps only: the watch's API reads, the push (which authenticates that one `git push` with `-c http…extraheader`), and the dashboard sync. Install, build and the gate chain run without it. Gate W3 checks this with `ops/write-job.mjs`.
