...
- **One dashboard issue.** `--dashboard` keeps a single issue, `[watch] Freshness dashboard`, edited in place, under the same trust rules as before. No per-event issues are opened for assessed repositories; new-repository candidates are listed on the dashboard.
- **Weekly digest.** The feed gets at most one watch entry per ISO week, listing the crossings opened and resolved that week.
...
node watch/watch.mjs --apply --dashboard   # what the scheduled run does: also sync the one dashboard issue
...
`--issues` and `--backfill-since-pin` were the scheduled path until 2026-09-24 and filed issues #1 to #12. Since 2026-09-25 (freshness contract FR-D.4) they are usage errors (exit 2) that name `--dashboard`. The dedupe and trust code they used stays in `watch.mjs`, because gate W tests it and the dashboard reuses it. What they did, as of v1.2.0:
