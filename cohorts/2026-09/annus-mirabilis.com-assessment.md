...
| 2 | Rust | Fails as written; set aside | Primary language TypeScript. Rust is present but small: one 366-line transport crate (`scripts/wasm-artifacts/fs-annus-wasm`, plus 530 lines of tests) that compiles FrankenSim sources by reference, and one test fixture **[Counted, High]**. The brief for this packet directs that the repository be assessed as a website, the way the pinned 44 assessed four TypeScript website repositories, so the predicate is set aside here by instruction, not by the analyst. A reviewer applying the rule literally would reject. |
