...
| 2 | Rust (primary language, or a Rust port or tool) | **Fail** | GitHub reports HTML; the languages API lists no Rust. The tree holds no Rust source and no `Cargo.toml`, and the failure rests on that. The only Rust-derived file is a vendored build artifact, `vendor/bv_graph_bg.wasm` with its wasm-bindgen glue `vendor/bv_graph.js`, which `vendor/MANIFEST.json` attributes to "bv-graph-wasm (this repository)" and says is "not yet reproducibly tied to source"; this snapshot does not establish the source repository. A compiled dependency copied into a static site does not make the site a Rust project. **[Verified, High]** |
