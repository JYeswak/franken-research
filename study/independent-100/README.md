...
- **No identity resolution.** Pseudonymous accounts appear under their handle and the name shown on the list, with no gendered pronouns. Routes and handles are those on the curator's public list. A URL may appear in a pseudonymous record only under one of four rules, and gate N checks every URL against them:
  ...
  - **Withheld everywhere.** A site that a pseudonymous account links from its X profile and that its record does not use, and any site named after a pseudonymous handle, appears nowhere in the study: not in another person's record, a deep dive, an evidence file or a page. A claim about someone else that rested on such a site was given another primary source or dropped.
...
- **Nothing about our own deployment.** Tools we use are named where they are public and studied on this site, but the study does not say how we configure or run them, what our own systems lack or have not checked, their internal sizes or thresholds, or our internal plans. Where a record once compared a person's work with our setup, it now states the general lesson from the person's work.
...
- `evidence/x-reads.jsonl`: one X API read per line, 2026-09-25. A post row has `handle`, `kind` ("post"), `id`, `url`, `fetched_at`, `text` and `linked_urls` (the expanded URLs in the post, less any withheld site). A profile row has `handle`, `kind` ("profile"), `fetched_at`, `profile_url` and `description_urls` (only those the record uses; otherwise null or empty), and `withheld_site_sha256`: the SHA-256 of each withheld site's key (the lowercased host without "www.", plus the first path segment on a platform such as substack.com), so gate N can find a withheld site without the study naming it. No profile text.
