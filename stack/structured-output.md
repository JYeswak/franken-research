...
- **Run one conformance suite against every backend**: outlines `tests/backends/`; a fresh read on 2026-09-23 of https://raw.githubusercontent.com/dottxt-ai/outlines/main/tests/backends/test_backends.py showed it importing the outlines-core, llguidance and xgrammar backends and testing JSON-schema and regex logits processors across them. Starter kit: none. [Verified] (ecosystem/pickup/_evidence/structured-output.md:16 "Per-backend conformance tests: run the same constraint semantics against every engine adapter.")
...
## Build only if
- Build the test harness the ecosystem lacks, not a new engine: a generative differential fuzzer (random grammar, compare masks against a reference parser). [Verified] (ecosystem/pickup/_evidence/structured-output.md:35 "A clean-room project should add one; the ecosystem doesn't demonstrate it.")
- Build an engine only if your runtime cannot link the C++/Python engines at all, as with franken_nlp's pure-Rust single-model runtime; that grammar layer exists as code but its guarantee has not been observed. [Maintainer claim] (packets/franken_nlp-assessment.md:91 "the construction guarantee is a design property, not an observed one")
