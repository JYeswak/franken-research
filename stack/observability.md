...
## Bottom line
Inference, medium confidence: record agent runs with an existing tracing stack (Langfuse, Arize Phoenix or Comet Opik, instrumented through OpenTelemetry packages such as OpenLLMetry or OpenLIT), and do not build your own trace store. Check the license first if you will host tracing for others: Phoenix is under the Elastic License 2.0, which forbids offering it as a hosted service, and Langfuse keeps its enterprise directories under a separate enterprise license. Wrap it, because "OpenTelemetry-compatible" is mostly asserted rather than tested in this category: before you trust an integration, check that the spans it emits carry the attributes you depend on, that streamed calls close their spans, and that your cost numbers use an audited price table.

## Adopt, do not rebuild
- **langfuse/langfuse**: a self-hostable platform for traces, nested spans, prompt and response logging, cost tracking, datasets and evals. License: MIT with conditions: the core is MIT, but code under the `ee/`, `web/src/ee/` and `worker/src/ee/` directories needs a Langfuse Enterprise License, so stay out of those paths unless you hold one. Its CI diffs the server-generated API spec against the SDK and audits model prices daily. [Verified] (ecosystem/pickup/_evidence/observability.md:7 "trace capture, span hierarchies, prompt/response logging, cost tracking, datasets + evals in one OSS platform (MIT)"; ecosystem/pickup/_evidence/observability.md:32 "sdk-api-spec.yml, openapi-export-check.yml"; stack/licenses.tsv:46 "have a valid Langfuse Enterprise License"; stack/licenses.tsv:46 "and/or 'worker/src/ee/' directories")
- **Arize-ai/phoenix**: observability and evaluation built on OpenTelemetry, with its trace view checked by browser tests. License: source-available under the Elastic License 2.0 (ELv2), which bars providing the software to third parties as a hosted or managed service; its `phoenix-otel` and `phoenix-client` packages are Apache-2.0, and `phoenix-evals` is ELv2. The quoted clause restricts providing it to third parties as a service; it does not mention running it for your own team. [Verified] (ecosystem/pickup/_evidence/observability.md:9 "packages include phoenix-otel (OTel-native) and phoenix-evals"; ecosystem/pickup/_evidence/observability.md:34 "Phoenix runs playwright.yaml for UI trace-view verification"; stack/licenses.tsv:6 "You may not provide the software to third parties as a hosted or managed service"; stack/licenses.tsv:6 "packages/phoenix-otel and packages/phoenix-client are Apache-2.0, packages/phoenix-evals is ELv2")
...
- **Audit the price table on a schedule**: a daily job checks the default model prices that cost numbers are computed from. Starter kit: none. [Verified] (ecosystem/pickup/_evidence/observability.md:33 "cost tracking needs its own scheduled correctness job — prices change under you")
- **Demote stale cost claims**: the companion proposes that a cost claim whose price-table audit is more than seven days old drops to inference. Starter kit: B8. [Inference] (ecosystem/pickup/pickup-observability.md:167 "whose price-table audit is older than 7 days is demoted to T3")
...
- The licenses do not justify building either. If you will run tracing as a service for customers, Phoenix's Elastic License 2.0 rules out its server, but Opik (Apache-2.0), Langfuse's MIT core outside its `ee/` directories, and Phoenix's own Apache-2.0 `phoenix-otel` instrumentation package remain available, so switch incumbents rather than write a trace store. [Verified] (stack/licenses.tsv:15 "comet-ml/opik Apache-2.0 permissive"; stack/licenses.tsv:46 "All content that resides under the 'ee/'"; stack/licenses.tsv:6 "packages/phoenix-otel and packages/phoenix-client are Apache-2.0")
