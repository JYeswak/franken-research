...
- **modelcontextprotocol/python-sdk**: the official Python server and client SDK, with its process read in the pack (pinned conformance package, per-revision expected-failure files, one conformance leg per wire revision). [Verified] (ecosystem/pickup/_evidence/mcp.md:8 "official Python SDK (server+client); pushes same-day"; ecosystem/pickup/_evidence/mcp.md:20 "Official protocol conformance harness run in CI, separate from unit tests")
...
- None of these packets records a run of the official conformance suite; a search of `packets/` for the string `modelcontextprotocol` on 2026-09-23 (`grep -c modelcontextprotocol packets/*.md`) returned no matches. [Verified] (packets/franken_snowflake-assessment.md:83 "stdio handshake/parity integration test exists per docs/RELEASE.md")
