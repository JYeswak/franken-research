...
- **modelcontextprotocol/python-sdk**: the official Python server and client SDK; its CI runs the official conformance suite at a pinned version and names failing scenarios in expected-failure files per spec revision. [Verified] (ecosystem/pickup/_evidence/mcp.md:8 "official Python SDK (server+client); pushes same-day"; ecosystem/pickup/_evidence/mcp.md:20 "python-sdk pins harness version in workflow env"; ecosystem/pickup/_evidence/mcp.md:21 "Expected-failures baseline files instead of skipping hard scenarios")
