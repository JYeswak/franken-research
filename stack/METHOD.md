...
9. **Check the license of everything you tell a builder to adopt.** "Do not rebuild, adopt X" is bad advice if X's license blocks the builder's product. Every `owner/repo` named in "Adopt, do not rebuild" has a row in `stack/licenses.tsv` from a dated, read-only GitHub API read (`gh api repos/<owner>/<repo> --jq .license`, plus the LICENSE file when the API says NOASSERTION or other). Columns: `repo` · `spdx` · `license_class` (`permissive` | `weak-copyleft` | `strong-copyleft` | `network-copyleft` | `source-available` | `permissive-with-conditions` | `none` | `unknown`) · `checked` (date) · `command` · `note`. When the class is anything but `permissive`, the Adopt bullet must name the license (e.g. "AGPL-3.0: network use triggers source release") so the constraint is in front of the reader, and "Build only if" must say whether it could justify building. "No license constraint" may only be written when the table shows one was checked. The site shows each adopted project's license next to it. Gate K fails if an adopted `owner/repo` has no row, or if a non-permissive license is not named in its bullet.
