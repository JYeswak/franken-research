...
6. **Deploy.** [deploy.yml](../.github/workflows/deploy.yml) runs on every push to `main`, after
   ...
   on demand. It runs the full gate chain on the exact commit, deploys `site/` to Cloudflare Pages
   only if every gate passes, then checks that the homepage and one brief return 200 and the
   homepage carries the committed `<title>`. A failed gate or smoke test fails the run, and the site
   stays on the previous deploy.
