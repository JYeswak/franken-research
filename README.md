...
| W3 | The freshness contract ([watch/freshness/SPEC.md](watch/freshness/SPEC.md)) holds offline: every MUST clause has a passing case or a documented discrepancy, every planted mutant makes its named cases fail, `watch/freshness/REPORT.md` equals a fresh render, every generated artifact in `ops/schedule.tsv` is refreshed by its workflow, the watch runs as a read-only build job and a publish job that alone can write and runs no dependency code, both only on `main`, with the dashboard sync after the push, and every brief's live card equals a fresh render from `watch/live.json`. |
