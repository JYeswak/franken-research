...
| W3 | The freshness contract ([watch/freshness/SPEC.md](watch/freshness/SPEC.md)) holds offline: every MUST clause has a passing case or a documented discrepancy, every planted mutant makes its named cases fail, `watch/freshness/REPORT.md` equals a fresh render, every generated artifact in `ops/schedule.tsv` is refreshed by its workflow, the watch job runs only on `main` with its token kept out of install, build and gate steps and its dashboard sync after the push, and every brief's live card equals a fresh render from `watch/live.json`. |
