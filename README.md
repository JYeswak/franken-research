...
| W3 | The freshness contract ([watch/freshness/SPEC.md](watch/freshness/SPEC.md)) holds offline: every MUST clause has a passing case or a documented discrepancy, every planted mutant makes its named cases fail, `watch/freshness/REPORT.md` equals a fresh render, every generated artifact in `ops/schedule.tsv` is refreshed by its workflow, every workflow keeps write tokens out of jobs that run dependency code (the watch and the discovery sweep each run a read-only build job and a publish job that alone can write, both only on `main`, with the issue sync after the push), no secret but `GITHUB_TOKEN` appears outside the three reviewed deploy steps, and every brief's live card equals a fresh render from `watch/live.json`. |
...
| N | The Independent 100 study pages equal a fresh `node site/scripts/make-study.mjs`, number exactly the 99 people plus the index plus the deep dives, and come from a `study/independent-100/people.jsonl` that keeps its schema (key order, https links, evidence labels, scores 0 to 5); the study sources and pages carry no local path, email address, phone number, image, X API field name or internal ticket id, no pseudonymous record uses a gendered pronoun, no rendered page shows the text `undefined`, `null` or `NaN`, every cited X post, X API read and GitHub license read has a committed row in `study/independent-100/evidence/`, and every URL in a pseudonymous record passes one of the four routes in the study README (x.com under the handle, a GitHub or Hugging Face owner equal to the handle, a one-hop link from the listed X account, a page that names the handle), and no site withheld from a pseudonymous record (its X profile links the record does not use, or a site named after the handle) appears anywhere in the study. |
| T | `site/llms.txt`, the site summary for language models ([llms.txt](https://fr.zeststream.ai/llms.txt)), equals a fresh `bun run build:llms` from the site's own data, pages, feeds and packets; every link in it is on this site or its source repository and resolves to a shipped file, and every linked page is in the sitemap. |
...
| `site/` | The static site served at fr.zeststream.ai. Runs from `file://` with no build step; `site/packets/` and `site/RULEBOOK.md` are byte-identical copies checked by gate A, `site/stack/` and `site/rigor/` are generated from `stack/` by `node site/scripts/make-stack.mjs` (gate K4 checks they match), `site/study/` is generated from `study/` by `node site/scripts/make-study.mjs` (gate N checks it matches), and `site/llms.txt` is generated from the site by `node site/scripts/make-llms.mjs` (gate T checks it matches). |
